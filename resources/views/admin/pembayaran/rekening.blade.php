...
@section('page_title', 'Kelola Rekening')
...
<div class="page-header">
    ...
    <div style="display:flex;gap:0.75rem;">
        <a href="{{ route('admin.pembayaran.index') }}" class="btn btn-outline">
            <i class="ri-arrow-left-line"></i> Kembali ke Monitor
        </a>
    </div>
</div>

{{-- Info Event --}}
<div style="background:linear-gradient(135deg,var(--navy),var(--navy-light));border-radius:var(--radius-lg);padding:1.25rem 1.5rem;margin-bottom:1.5rem;color:white;display:flex;gap:2rem;flex-wrap:wrap;" data-aos="fade-down">
    <div>
        <div style="font-size:0.68rem;text-transform:uppercase;letter-spacing:0.08em;color:rgba(255,255,255,0.5);">Panitia</div>
        <div style="font-weight:600;">{{ $event->panitia->nama ?? '-' }}</div>
    </div>
    <div>
        <div style="font-size:0.68rem;text-transform:uppercase;letter-spacing:0.08em;color:rgba(255,255,255,0.5);">Tanggal</div>
        <div style="font-weight:600;">{{ $event->tanggal->format('d M Y') }}</div>
    </div>
    <div>
        <div style="font-size:0.68rem;text-transform:uppercase;letter-spacing:0.08em;color:rgba(255,255,255,0.5);">Biaya</div>
        <div style="font-weight:700;color:var(--orange);">Rp {{ number_format($event->biaya, 0, ',', '.') }}</div>
    </div>
    <div>
        <div style="font-size:0.68rem;text-transform:uppercase;letter-spacing:0.08em;color:rgba(255,255,255,0.5);">Status Rekening Aktif</div>
        <div style="font-weight:600;">{{ $rekenings->where('is_active', true)->count() }} dari {{ $rekenings->count() }}</div>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1.5fr;gap:1.5rem;" data-aos="fade-up">
    ...
    <div class="card" style="align-self:start;">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.pembayaran.rekening.store', $event->id_event) }}">
                @csrf
    ...
    <div>
        <div class="card">
            <div class="card-header">
                <div style="font-weight:700;color:var(--navy);">
                    <i class="ri-bank-card-line"></i> Rekening Tersimpan ({{ $rekenings->count() }})
                </div>
                    ...
                    <div style="border:2px solid {{ $r->is_active ? 'var(--navy-pale)' : 'var(--gray-100)' }};
                        ...
                        <div style="display:flex;align-items:center;gap:0.75rem;min-width:0;">
                            <div style="width:42px;height:42px;border-radius:var(--radius);
                                        ...
                                        color:{{ $r->is_active ? 'white' : 'var(--gray-400)' }};
                        ...
                        <div style="display:flex;gap:0.5rem;flex-shrink:0;">
                            {{-- Toggle --}}
                            <form method="POST"
                                  action="{{ route('admin.pembayaran.rekening.toggle', [$event->id_event, $r->id_rekening]) }}">
                                @csrf
                                <button type="submit"
                                        class="btn btn-sm {{ $r->is_active ? 'btn-success' : 'btn-outline' }}"
                            ...
                            {{-- Hapus --}}
                            <form method="POST"
                                  action="{{ route('admin.pembayaran.rekening.destroy', [$event->id_event, $r->id_rekening]) }}">
                                @csrf @method('DELETE')
        ...
        <div class="alert alert-navy" style="margin-top:1rem;">
            ...
            <div>
                <strong>Catatan Admin:</strong> Hanya rekening berstatus <strong>Aktif</strong> yang tampil kepada peserta saat memilih transfer. Nonaktifkan rekening yang sudah tidak digunakan.
            </div>
        </div>
    </div>
</div>
